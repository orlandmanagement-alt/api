...
import { getClientInvites, postClientInvite } from "../../services/client/client_invites_service.js";
...
export async function onRequestGet({ request, env }){
  ...
  return json(200, "ok", await getClientInvites(env, projectId));
}
...
export async function onRequestPost({ request, env }){
  ...
  const result = await postClientInvite(env, body);
  if (result?.error) {
