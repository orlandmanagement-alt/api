...
export async function onRequestPost({ request, env }){
  ...
  let email_hash = "";
  try{ email_hash = await sha256Base64(email + "|" + (env.HASH_PEPPER||"")); }
  catch{ email_hash = email; }
  ...
  const rid = await ensureRole(env, "client");
...
export async function onRequestPut({ request, env }){
  ...
  if(action==="disable" || action==="enable"){
    ...
    await env.DB.prepare("UPDATE users SET status=?, updated_at=? WHERE id=?").bind(st, now, user_id).run();
