...
export async function getClientBookings(env, projectId = "") {
  const rows = await listProjectBookings(env, projectId);
  return {
    items: rows.map(row => ({
      id: row.id,
...
export async function postClientBooking(env, body) {
...
export async function patchClientBooking(env, body) {
